...
assert (
    max(y_pred.flatten()) <= 1 and min(y_pred.flatten()) >= 0
), "The inference function does not work properly. Normalization issue. for y_pred (min = {}, max = {})".format(
    min(y_pred.flatten()), max(y_pred.flatten())
)
