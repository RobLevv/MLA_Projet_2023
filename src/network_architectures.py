...
encoder_layers = Sequential(
    ...
    Conv2d(
        in_channels=IMG_SIZE[0], out_channels=16, kernel_size=4, stride=2, padding=1
    ),
    BatchNorm2d(16),
...
decoder_layers = Sequential(
    ...
    ConvTranspose2d(
        in_channels=512 + N_ATTRIBUTES,
        out_channels=512 + N_ATTRIBUTES,
        kernel_size=4,
        stride=2,
        padding=1,
    ),
    ...
    ConvTranspose2d(
        in_channels=256 + N_ATTRIBUTES,
        out_channels=128 + N_ATTRIBUTES,
        kernel_size=4,
        stride=2,
        padding=1,
    ),
    ...
    ConvTranspose2d(
        in_channels=128 + N_ATTRIBUTES,
        out_channels=64 + N_ATTRIBUTES,
        kernel_size=4,
        stride=2,
        padding=1,
    ),
    ...
    ConvTranspose2d(
        in_channels=64 + N_ATTRIBUTES,
        out_channels=32 + N_ATTRIBUTES,
        kernel_size=4,
        stride=2,
        padding=1,
    ),
...
if __name__ == "__main__":
    ...
    print(
        ...
        sum(p.numel() for p in decoder_layers.parameters() if p.requires_grad),
    )
    print(
        "Total number of trainable parameters:",
        sum(
            p.numel()
            for p in decoder_layers.parameters()
            if p.requires_grad and p.is_leaf
        ),
    )
    print(
        "Total number of non-trainable parameters:",
        sum(p.numel() for p in decoder_layers.parameters() if not p.requires_grad),
    )
