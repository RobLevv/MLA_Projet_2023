...
def get_celeba_dataset():
    return ImgDataset(
        attributes_csv_file="data/Anno/list_attr_celeba.txt",
        img_root_dir="data/Img_processed",
        transform=None,
    )
...
def get_celeba_dataset_lite():
    return ImgDataset(
        attributes_csv_file="data/Anno/list_attr_celeba_lite.txt",
        img_root_dir="data/Img_processed_lite",
        transform=None,
    )
...
class ImgDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, attributes_csv_file: str, img_root_dir: str, transform):
        """
        Arguments:
    ...
    def __getitem__(self, idx):
        ...
        img_name = os.path.join(self.root_dir, self.attributes_df.iloc[idx, 0])
...
if __name__ == "__main__":
    ...
    for idx in range(N):
        ...
        ax[0, idx].axis("off")
        ax[0, idx].set_title(
            "Image {}".format(dataset.attributes_df.iloc[idx, 0]), fontsize=8
        )
